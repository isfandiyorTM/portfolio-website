...
const QUESTIONS = [
  ...
  { q:"Which Dart keyword is constant at compile time?", options:["final","static","const","var"], answer:2 },
...
export default function Quiz() {
  ...
  const restart = () => { setCurrent(0); setSelected(null); setScore(0); setDone(false); setAnswered(false); };

  if(done) return (
    <div style={{ textAlign:"center", padding:"24px 0" }}>
      <div style={{ fontSize:"clamp(40px,12vw,56px)", marginBottom:"12px" }}>{score>=7?"🏆":score>=5?"👍":"📚"}</div>
      <div style={{ fontFamily:"var(--font-display)", fontSize:"clamp(28px,8vw,40px)", color:"var(--green)", marginBottom:"6px" }}>{score}/{questions.length}</div>
      <div style={{ fontFamily:"var(--font-mono)", fontSize:"clamp(11px,3vw,13px)", color:"var(--text-muted)", marginBottom:"28px", letterSpacing:"1px" }}>
        {score>=7?"FLUTTER EXPERT! 🔥":score>=5?"GOOD JOB! KEEP LEARNING 💪":"KEEP READING FLUTTER DOCS! 📖"}
      </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <span style={{ fontFamily:"var(--font-mono)", fontSize:"11px", color:"var(--text-muted)", letterSpacing:"2px" }}>{current+1} / {questions.length}</span>
        <span style={{ fontFamily:"var(--font-display)", fontSize:"16px", color:"var(--green)" }}>⭐ {score}</span>
      </div>
      ...
      {/* Question */}
      <div style={{ background:"var(--bg)", border:"1px solid var(--border)", padding:"16px 20px", fontFamily:"var(--font-mono)", fontSize:"clamp(13px,3vw,15px)", lineHeight:1.7, color:"var(--text)" }}>
        <span style={{ color:"var(--green)", marginRight:"8px" }}>//</span>{q.q}
      </div>

      {/* Options — 2 col on desktop, 1 col on mobile */}
        ...
        {q.options.map((opt,i)=>{
          ...
          return(
            <button key={i} onClick={()=>handleAnswer(i)} style={{
              ...
              fontSize:"clamp(12px,3vw,13px)", cursor:answered?"default":"pointer",
